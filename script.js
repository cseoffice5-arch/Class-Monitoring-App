/* ----------------------------------------------------
   script.js — FINAL, UPDATED as per your guidelines
   ---------------------------------------------------- */
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  // Watermark text
  const mw = document.getElementById("m_teacher_watermark");
  const kw = document.getElementById("k_teacher_watermark");

  document.getElementById("m_teacher").addEventListener("change", e => {
    mw.textContent = e.target.value ? "Selected teacher: " + e.target.value : "";
  });

  document.getElementById("k_teacher").addEventListener("change", e => {
    kw.textContent = e.target.value ? "Selected teacher: " + e.target.value : "";
  });
});


/* ----------------------------------------------------
   SIMPLE POST FUNCTION (Avoids CORS preflight)
   ---------------------------------------------------- */
...
document.getElementById("missedForm").addEventListener("submit", async function (e) {
  ...
  if (!payload.date || !payload.department || !payload.course || !payload.room || !payload.teacherInitial) {
    return alert("Please fill Date, Department, Course, Room, and Teacher.");
  }
