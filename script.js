/* ------------------------
   script.js — final (client)
   ------------------------ */

/* ====== SET YOUR DEPLOYED WEB APP URL ======
   Replace with your deployed Apps Script URL if it changes.
*/
...
/* ---------- DATA ARRAYS ---------- */
/* Paste your full TEACHERS, ROOMS and TIMES arrays here (kept intact from your project) */
...
window.addEventListener('DOMContentLoaded', () => {
  // Populate selects (if your HTML uses plain selects)
  ...
  // If you have filter inputs, wire them (IDs used in your HTML)
  ...
  if (mTeacher) mTeacher.addEventListener('change', e => {
    const w = document.getElementById('m_teacher_watermark');
    if (w) w.textContent = e.target.value ? "Selected teacher: " + e.target.value : "";
  });
  if (kTeacher) kTeacher.addEventListener('change', e => {
    const w = document.getElementById('k_teacher_watermark');
    if (w) w.textContent = e.target.value ? "Selected teacher: " + e.target.value : "";
  });
});
...
if (missedForm) {
  missedForm.addEventListener('submit', async function (e) {
    e.preventDefault();
    const payload = {
      action: 'save_missed',
    ...
    try {
      ...
      if (result.status === 'success') {
        ...
        this.reset();
        const wm = document.getElementById('m_teacher_watermark');
  ...
  makeupForm.addEventListener('submit', async function (e) {
    ...
    try {
      ...
      if (result.status === 'success') {
        ...
        this.reset();
        const wm = document.getElementById('k_teacher_watermark');
        if (wm) wm.textContent = '';
        populateSelectList('k_teacher', TEACHERS);
        populateSelectList('k_room', ROOMS);
        populateSelectList('k_time', TIMES);
