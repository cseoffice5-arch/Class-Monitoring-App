...
/* =====================================================
    MISSED CLASS — EXACT COLUMN ORDER
   ===================================================== */
document.getElementById("missedForm").addEventListener("submit", async function (e) {
  ...
  // required fields
  if (!payload.date || !payload.department || !payload.course ||
      !payload.timeSlot || !payload.room || !payload.teacherInitial) {
    alert("Please fill Date, Department, Course, Time Slot, Room & Teacher.");
    return;
  }

  const result = await postForm(payload);
  if (result.status === "success") {
    alert("Missed Class Saved Successfully!");
    this.reset();
    populateSelect("m_time", TIMES);
    populateSelect("m_room", ROOMS);
    populateSelect("m_teacher", TEACHERS);
...
/* =====================================================
    MAKEUP CLASS — EXACT COLUMN ORDER
   ===================================================== */
document.getElementById("makeupForm").addEventListener("submit", async function (e) {
  ...
  const payload = {
    ...
    department: document.getElementById("k_dept_makeup").value,
  ...
  // required fields
  if (!payload.scheduleDate || !payload.department || !payload.course ||
      !payload.teacherInitial || !payload.makeupDate ||
      !payload.makeupTime || !payload.makeupRoom) {
  ...
  const result = await postForm(payload);
  if (result.status === "success") {
    alert("Makeup Class Saved Successfully!");
    this.reset();
    populateSelect("k_time", TIMES);
    populateSelect("k_room", ROOMS);
    populateSelect("k_teacher", TEACHERS);
