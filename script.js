...
document.getElementById("makeupForm").addEventListener("submit", async function(e) {
  ...
  const payload = {
    action: "save_makeup",
    scheduleDate: document.getElementById("k_schedule").value.trim(),
    ...
    makeupDate: document.getElementById("k_date").value.trim(),
  ...
  if (!payload.date || !payload.department || !payload.course || !payload.makeupRoom || !payload.teacherInitial) {
    return alert("Please fill Date, Department, Course, Room, and Teacher.");
  }
